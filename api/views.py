...
def usuario_publico(request):
    """Endpoint público para verificar usuario (sin requiere autenticación)"""
...
def reportes(request):
    """Listado de reportes (template antiguo)"""
...
def detalle_reporte(request, id):
    """Detalle de un reporte específico (template antiguo)"""
...
def reportes_usuario(request):
    ...
    # Obtener información del usuario y algunos datos contextuales
    context = {
        'usuario': request.user,
        'total_reportes': ReporteFuga.objects.count(),
        'mis_reportes': ReporteFuga.objects.filter(owner=request.user).count(),
    }
...
def test_api(request):
    """Página de prueba de la API (acceso público para debugging)"""
    return render(request, "users/test_api.html")


@require_http_methods(["GET", "POST"])
def login_view(request):
    """Formulario de inicio de sesión personalizado para usuarios."""
    if request.user.is_authenticated:
        return redirect('reportes_usuario')
...
def home(request):
    """Página de inicio que redirige a reportes-app"""
    if request.user.is_authenticated:
        ...
        return render(request, 'users/home.html', {
            'message': 'Bienvenido al Sistema de Reportes de Fugas'
        })
...
def registro(request):
    ...
    if request.method == 'POST':
        ...
        if not username or len(username) < 3:
            errors.append('Usuario debe tener mínimo 3 caracteres')
